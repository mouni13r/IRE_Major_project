...
class HSMM(nn.Module):
    ...
    def temp_ar_bs(self, templt, row2tblent, row2feats, h0, c0, srcfieldenc, len_lps,  K,
        ...
        nfeats = 4
        with torch.no_grad():
            inps = Variable(torch.LongTensor(K, nfeats))
            ...
            for ell in range(self.L+1):
                ...
                for k in range(2*K):
                    ...
                    if wrd == self.eop_idx and ell > 0 and (not final_state or curr_hyps[anc][0] == eos_idx):
                        ...
                        ttt = anc.cpu().long()
                        print(ttt, ttt.type())
                        nulens[len(new_hyps)][0] = (curr_lens[ttt][0]+1)
                        if wrd >= self.decoder.out_features: # a copy
                            tblidx = wrd - self.decoder.out_features
                            inps.data[len(new_hyps)].copy_(row2feats[tblidx])
                        else:
                        ...
                        print("anc", anc, curr_hyps)
                        #toto = curr_hyps[anc.long()]
                        print(toto)
...
def make_masks(src, pad_idx, max_pool=False):
    ...
    #print(fieldmask)
    avgmask = (~( fieldmask)).float() # 1s where not padding
    if not max_pool:
...
if __name__ == "__main__":
    ...
    def train(epoch):
        ...
        for batch_idx in range(nmini_batches):
            ...
            #print(beta_star[0].shape, init_logps.shape)
            log_marg = logsumexp1(beta_star[0] + init_logps)
            log_marg = log_marg.sum() # bsz x 1 -> 
            #print(neglogev)
            #print(type(log_marg))
            neglogev -= log_marg
    ...
    def label_train():
        ...
        for i in range(len(corpus.train)):
            ...
            for b in range(bsz):
                temp = corpus.dictionary.idx2word
                #print(temp[0])
                words = [temp[w] for w in x[:, b]]
                #print(words)
                #print(seqs[b])
                for ( end, start, label) in seqs[b]:
                    #print(words[0], "word")
                    if type(start) == "<class 'torch.Tensor'>" :
                        start = start.detach()
                    if type(end) == "<class 'torch.Tensor'>" :
                        end = end.detach()
    ...
    def gen_from_srctbl(src_tbl, top_temps, coeffs, src_line=None):
        ...
        with torch.no_grad():
            srcenc, srcfieldenc, uniqenc = net.encode(src_b, None, uniq_b)
        ...
        temp = inits.size(1)//2
        print(temp)
        h0, c0 = torch.tanh(inits[:, :temp]), inits[:, temp:]
